...
class KV extends EventEmitter {
    ...
    function mapfn(row, next) {
      ...
      if (row.value.k !== undefined) {
        self.xdb.get(row.value.k, function (err, ops) {
          ...
          var doc = {}; (ops || []).forEach(function (op) { doc[op.key] = op.type })
          ...
          self.xdb.put(row.value.k, doc, function (err) {
            if (!err)
              self.emit('update', row.value.k, row.value.v, row)
            next(err)
          })
        })
        ...
        self.xdb.get(row.value.d, function (err, ops) {
          ...
          function onput(err) {
            if (!err)
              self.emit('remove', row.value.d, row)
            next(err)
          }
        })
      } else
  ...
  put(key, value, opts, cb) {
    ...
    if (!cb)
      cb = noop
    var doc = Object.assign({}, opts.fields || {}, { k: key, v: value })
  ...
  del(key, opts, cb) {
    ...
    if (!cb)
      cb = noop
    var doc = Object.assign({}, opts.fields || {}, { d: key })
    self._put(key, doc, opts, function (err, node) {
      cb(err, node)
      if (!err)
        self.emit('del', key, node)
    })
  }
  get(key, opts, cb) {
    var self = this
  ...
  createReadStream(opts) {
    ...
    self.dex.ready(function () {
      if (opts.live) {
        ...
        pump(self.xdb.createReadStream(xopts), stream)
    })
  ...
  createHistoryStream(key, opts) {
    ...
    function onget(err, doc) {
      ...
      if (doc.identity)
        rdoc.identity = doc.identity
      if (doc.signature)
        rdoc.signature = doc.signature; (doc.links || []).forEach(function (link) {
          if (!has(seen, link))
            queue.push(link)
        })
      stream.push(rdoc)
  ...
  batch(rows, opts, cb) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (row.type === 'put') {
        batch.push({
          value: Object.assign({}, row.fields || {}, { k: row.key, v: row.value }),
          links: row.links
        })
      } else if (row.type === 'del') {
        batch.push({
          value: Object.assign({}, row.fields || {}, { d: row.key }),
          links: row.links
        })
      } else if (row.type) {
        return ntick(cb, 'batch type not recognized')
      } else if (!row.type)
        return ntick(cb, 'batch type not provided')
    }

    if (batch.every(hasLinks))
      return self.log.batch(batch, cb)
